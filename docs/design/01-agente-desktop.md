...
| **Domínio do site em foco** (implementado em 16/09/2026) | **UI Automation** (`IUIAutomation` por interop escrito à mão, sem dependência externa): `ElementFromHandle` na janela em foco → primeira `ToolBar` → primeiro `Edit` dela (omnibox do Chromium / urlbar do Firefox) → `UIA_ValueValuePropertyId`. A url crua é reduzida a **domínio registrável** ainda na máquina (`Privacy.SiteDomain`), descartando caminho, query, credencial e subdomínio. Busca DIRIGIDA (`FindFirst`), nunca varredura da árvore. **Não** liga acessibilidade de conteúdo web, não vê aba fora do foco e não abre histórico | Helper (thread STA própria, prazo de 1,5 s, disjuntor após 3 falhas) | No mesmo polling, e SÓ com navegador em foco |
| **Nome acessível da janela do navegador** (proteção, não coleta) | UIA `FindAll(Children)` + `UIA_NamePropertyId` — é onde o Chrome marca `(Modo anônimo)` e o Edge `(InPrivate)`. Lido MESMO com a coleta de sites desligada: sem ele, o rebaixamento automático em navegação anônima não acontece no Chrome atual | Helper | Junto da leitura acima |
| **Nome do arquivo aberto** | Extração do TÍTULO já mascarado (`Privacy.DocumentName`), lista fechada de extensões de documento; caminho no título vira só o último segmento | Helper | Sem API nova: é o título que já era coletado |
